...
Inline math renders with the mathjax extension locally and on the site, and with
asciidoctor-mathematical in the PDF: the mask latexmath:[2^n - 1] selects the low
latexmath:[n] bits.
...
A link to an anchor on another page names that page, so it resolves both in the
assembled PDF and on the site: see <<intro.adoc#intro>>. Written as
`+<<intro.adoc#intro>>+`, with no link text: the PDF build generates the text,
and xref_text_extension supplies it on the site.
