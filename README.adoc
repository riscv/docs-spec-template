...
4. **Enable template updates**:
   `.github/workflows/template-sync.yml` runs weekly (and on manual dispatch) and opens a pull request that pulls in template-owned file changes from `docs-spec-template` — see link:UPGRADING.md[UPGRADING.md]. It needs a `GHTOKEN` repository secret specifically: the workflow updates files under `.github/workflows/`, and `GITHUB_TOKEN` cannot push those regardless of the *Allow GitHub Actions to create and approve pull requests* setting in step 1. Complete `.template-version` (UPGRADING.md section 1) so the workflow knows your baseline; it skips itself with a warning until that file exists.
